...
")
 ...
 (b:inserts:dblock|moded
    (s-lex-format
     "b:elisp:pkg/fullUpdate :outLevel 1 :pkgAdoptionType \"${$fileType}\" :pkgName \"${$pkgName}\""))
  (b:insert:newLines|nuOf 1)

 (b:inserts:dblock|moded
    (s-lex-format
     "b:elisp:pkg:install/update :outLevel 1 :pkgAdoptionType \"${$fileType}\" :pkgName \"${$pkgName}\""))
 (b:insert:newLines|nuOf 1)

 (b:inserts:dblock|moded
    (s-lex-format
     "b:elisp:pkg:config/main :outLevel 1 :pkgAdoptionType \"${$fileType}\" :pkgName \"${$pkgName}\""))
 (insert "  )\n")
